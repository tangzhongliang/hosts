...
func TestGorm() {
	...
	//just test
	var err error
	err = GetDB().Create(&SnsEpAccount{AccountId: "aaaa", EPType: "slack"}).Error
	snserror.LogAndPanic(err)
	var accounts SnsEpAccount
	err = GetDB().Model(&SnsEpAccount{AccountId: "aaaa2", EPType: "slack2", Email: "sadfsdf"}).Find(&accounts).Error
